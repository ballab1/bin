...
#NAS DETAILS
nas_url="10.3.1.4" #url/IP to the NAS itself. Since it is likely running on the NAS just leave it to localhost
nas_name="bobb"  #Leave empty if you want to get the name from the NAS itself, otherwise provide a custom name
ups_group="" #For filtering metrics if more than one UPS, only necessary if capturing ups stats is enabled
