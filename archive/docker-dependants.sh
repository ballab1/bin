...
declare -A CONTAINERS CONTAINERS_REF IMAGES IMAGE_LAYERS IMAGE_REF NAMED_IMAGES REFS SPECIFIED_IMAGE_DEPENDENCIES
...
function findContainers()
{
    ...
    for ref in "${specifiedDockerRefs[@]}"; do
        ...
        for id in "$[!CONTAINERS[@]}"; do
            grep -qsE "$ref" <<< "${CONTAINERS[$id]}"  || echo "$id"
        done
...
function findMatches()
{
    ...
    MATCHED_CONTAINER_IDS=( $(findContainers "${specifiedDockerRefs[@]}") )
    reportMatchingContainers


    # determine the image IDs coresponding to specified dependents
    mapfile -t MATCHED_IMAGE_IDS < <(findImageIds "${specifiedDockerRefs[@]}")
    ...
    verbose "    Detected ${#MATCHED_IMAGE_IDS[*]} images associated with specified references"
    return 0
}
...
{
    [ "${#MATCHED_IMAGE_IDS[*]:-}" -ne 0 ] && printf '%s\n' "${MATCHED_IMAGE_IDS[@]}"
...
function populateCONTAINERS()
{
    ...
    CONTAINERSJSON="$(curl --silent --unix-socket /var/run/docker.sock http://localhost/containers/json?all='true')" || return 1
    readonly CONTAINERSJSON

    # Populate the container info hash
    CONTAINERS=()
    for id in $(jq -r '.[]|.Id' <<< "$CONTAINERSJSON"); do
        CONTAINERS[$id]=$( jq '.[]|.Id' <<< "$CONTAINERSJSON")
    done
    readonly CONTAINERS
...
function populateIMAGES()
{
    ...
    IMAGES=()
    for id in $(jq -r '.[]|.Id' <<< "$IMAGESJSON"); do
        IMAGES[$id]=$( jq '.[]|select(.Id == '$id')' <<< "$IMAGESJSON")
    done
    readonly IMAGES

    # Populate the named image info hash
    NAMED_IMAGES=()
    for id in $(jq -r '.[]|select(.RepoTags[0] != "<none>:<none>")|.Id' <<< "$IMAGESJSON"); do
        NAMED_IMAGES[$id]=$( jq '.[]|select(.Id == '$id')' <<< "$IMAGESJSON")
    done
    readonly NAMED_IMAGES
